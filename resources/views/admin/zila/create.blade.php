...
@section('content')
    <div class="row">
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.zila.store') }}" name="addfrm" id="addfrm" method="POST"
                        enctype="multipart/form-data" autocomplete="off">
                        @csrf

                        @isset($zila)
                            <input type="hidden" name="zila_id" id="zila_id" value="{{ $zila->id }}">
                        @endisset
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3 controls">
                                    ...
                                    <select class="form-control select2" name="state_id" id="state_id">
                                        @if (isset($zila) && isset($zila->state))
                                            <option value="{{ $zila->state->id }}" selected>{{ $zila->state->name }}</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name" value="{{ old('name', isset($zila) ? $zila->name : '') }}">
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 controls">
                                    ...
                                    <div class="form-check form-switch form-switch-md mb-3" dir="ltr">
                                        <input type="checkbox" name="status" class="form-check-input" {{ isset($zila) && $zila->status === 1 ? 'checked' : '' }}>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <a href="{{ route('admin.zila.index') }}" class="btn btn-secondary w-md button-responsive">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
